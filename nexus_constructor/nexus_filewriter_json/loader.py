...
"""
from nexus_constructor.data_model import Component, ComponentType, Rotation, Translation
...
def load_json_object_into_instrument_model(json_data: dict, model: InstrumentModel):
    ...
    if nx_sample is None:
        sample = Component(component_type=ComponentType.SAMPLE, name="Sample")
    else:
    ...
    named_components = {sample.name: sample}

    def process_component(nx_component):
        component, parent_name, dependent_transform_name = generate_component(
            nx_component
        )
    ...
    for child in nx_instrument["children"]:
        if (
            has_nx_class(child)
            and attribute_value(child, "NX_class") in component_classes
        ):
...
def attribute_value(json_object: dict, attribute_name: str):
    ...
    raise KeyError(
        "{} does not contain attribute {}".format(json_object, attribute_name)
    )


def generate_component_type(json_component: dict):
    return NexusDecoder.component_type_from_classname(
        attribute_value(json_component, "NX_class")
    )
...
def generate_component(json_component: dict):
    ...
    parent_name, dependent_transform_name = NexusDecoder.extract_dependency_names(
        dependent_on
    )
...
def generate_geometry_and_pixel_data(json_component: dict):
    ...
    for child in json_component["children"]:
        if child["name"] in ("shape", "pixel_shape", "detector_shape"):
            ...
            if attribute_value(child, "NX_class") == "NXcylindrical_geometry":
                ...
                assert (
                    len(cylinders) == 1
                ), "Cylindrical geometry only supports one cylinder"
                ...
                assert (
                    winding_order is not None
                ), "OFF geometry had no winding_order dataset"
                assert (
                    winding_order_indexes is not None
                ), "OFF geometry had no faces dataset"
                geometry = OFFGeometry(
                    vertices=vertices,
                    faces=NexusDecoder.unwound_off_faces(
                        winding_order, winding_order_indexes
                    ),
                )
    ...
    if (
        x_offsets is not None
        and y_offsets is not None
        and z_offsets is not None
        and detector_numbers is not None
    ):
        pixel_data = NexusDecoder.build_pixel_grid(
            x_offsets, y_offsets, z_offsets, detector_numbers
        )
...
def generate_transforms(json_component: dict):
    ...
    for child in json_component["children"]:
        if (
            has_nx_class(child)
            and attribute_value(child, "NX_class") == "NXtransformations"
        ):
