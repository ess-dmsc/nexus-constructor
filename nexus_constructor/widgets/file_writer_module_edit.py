...
class FileWriterModuleEdit(QWidget):
    ...
    def _handle_module_change(self, new_index: int):
        module_map = {
            WriterModules.F142.value: WriterModuleClasses.F142,
        ...
        if c_module_type != new_module_type:
            new_module = module_map[new_module_type].value(
                parent_node=self._module_container.module.parent_node,
                topic=self._module_container.module.topic,
                source=self._module_container.module.source,
            )
            module_index = self._module_container.module.parent_node.children.index(
                self._module_container.module
            )
            self._module_container.module.parent_node.children[
                module_index
            ] = new_module
    ...
    def _handle_more_or_less(self):
        self._module_container.module.expanded = (
            not self._module_container.module.expanded
        )
    ...
    def _populate_extra_attributes(self):
        ...
        if hasattr(self._module_container.module, "expanded"):
            ...
            self._second_line_widget = extra_config_map[
                self._module_container.module.writer_module
            ](self, self._module_container)
            self._second_line_widget.is_valid.connect(
                partial(self._module_validator.set_is_valid, self._second_line_widget)
            )
