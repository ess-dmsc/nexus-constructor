...
def test_GIVEN_nothing_WHEN_creating_nexus_model_THEN_creates_instrument_group_with_correct_nx_class():
    model = NexusModel()
    assert model.getInstrumentGroup().attrs["NX_class"] == instrument_name
...
def test_GIVEN_another_variable_WHEN_setting_instrument_group_THEN_variable_does_not_change():
    model = NexusModel()
    original = model.getInstrumentGroup()
    model.setInstrumentGroup("test")
    assert original == model.getInstrumentGroup()
