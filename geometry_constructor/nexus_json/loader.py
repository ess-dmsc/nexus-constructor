...
class Loader(QObject):
    ...
    def load_json_into_instrument_model(json_data: str, model: InstrumentModel):
        ...
        if nx_sample is None:
            ...
            sample, _, _ = Loader.generate_component(nx_sample)
        ...
        for child in nx_instrument['children']:
            if Loader.has_nx_class(child) and child['attributes']['NX_class'] in component_classes:
                component, parent_name, dependent_transform_name = Loader.generate_component(child)
    ...
    def generate_component(json_component: dict):
        ...
        geometry, pixel_data = Loader.generate_geometry_and_pixel_data(json_component)

        transforms, dependent_on = Loader.generate_transforms(json_component)
    ...
    def generate_geometry_and_pixel_data(json_component: dict):
        ...
        for child in json_component['children']:
            if child['name'] == 'geometry':
                geometry = None
                if not Loader.has_nx_class(child):
                    continue
    ...
    def generate_transforms(json_component: dict):
        ...
        for child in json_component['children']:
            if Loader.has_nx_class(child) and child['attributes']['NX_class'] == 'NXtransformations':
                name = child['name']
